...
class sey {
    ...
    run(config) {
        ...
        return _asyncToGenerator(function* () {
            ...
            const runner = new Runner(_this.moduleManager, currentConfig);

            runner.load();
            runner.populateFiles('publish', _this.options);

            const result = yield runner.run('publish', _this.options);
    ...
    build(options) {
        ...
        return _asyncToGenerator(function* () {
            ...
            const config = require(_this2.options.seyfile);

            if (Object.keys(config).length > 0) {
                yield _this2.run(config);
            }
        })();
    }
}
